...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/ycparkHW5" 
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        newp = form.newPriority.data
        lst = TodoItem.query.filter_by(description = item).first()
        lst.priority = newp
        db.session.commit()
        flash("Updated priority of item: " + item)
        return redirect(url_for('all_lists'))
...
def delete(lst):
    ...
    todos = TodoList.query.filter_by(title = lst).first()
    db.session.delete(todos)
    db.session.commit()
    flash("Successfully delted " + lst)
    return redirect(url_for('all_lists'))
